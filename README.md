...
今の金価格が **自分で決めた基準日と比べてどうか**、そして **過去の価格帯の中でどのあたりか** を
...
表示例
現在の金価格       23,743 円/g
基準日 2026/2/20   27,503 円/g   ← 日付は画面から自由に変更できます
基準日から         −3,760 円/g（−13.7%）
過去1年の位置      上位 62% → 平均
```
...
基準日は固定値ではなく、アプリの画面から変更します。
「基準日との比較」カードの日付入力で好きな日を選ぶか、その下のクイック選択
（1か月前・3か月前・6か月前・1年前）を押します。
選んだ日付はその端末のブラウザにだけ保存され、次に開いたときも引き継がれます。

* 何も選んでいないときは「1年前」を使います。既定値は `script.js` の `CONFIG.defaultBaselineDays` で変えられます。
* クイック選択の項目は `CONFIG.baselinePresets` で増減できます。
* 休場日を選んだ場合は直前の営業日の価格を使い、どの日を使ったかを画面に表示します。
* 店頭価格の履歴より前の日付を選んだ場合は市場価格（税抜）どうしで比較し、その旨を表示します。

基準日はユーザーごとの設定なので、データ側（`data/*.json`）や GitHub Actions には一切関与しません。
